...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            if ((self.normalization=='batchnorm') or (self.normalization=='layernorm')) and ((i+1)!=self.num_layers):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers, 1):
            if self.normalization=='batchnorm':
                activation, cache = affine_relu_bn_forward(activation, self.params['W'+str(i)], self.params['b'+str(i)],
                    self.params['gamma'+str(i)], self.params['beta'+str(i)], self.bn_params[i-1])
            elif self.normalization=='layernorm':
                activation, cache = affine_relu_ln_forward(activation, self.params['W'+str(i)], self.params['b'+str(i)],
                    self.params['gamma'+str(i)], self.params['beta'+str(i)], self.bn_params[i-1])
            else:
                activation, cache = affine_relu_forward(activation, self.params['W'+str(i)], self.params['b'+str(i)])
            if self.use_dropout:
                activation, dropout_cache = dropout_forward(activation, self.dropout_param)
                dropout_caches.append(dropout_cache)
            caches.append(cache)
            reg_loss += np.sum(np.square(self.params['W'+str(i)]))

        i = self.num_layers
        scores, cache = affine_forward(activation, self.params['W'+str(i)], self.params['b'+str(i)])
        caches.append(cache)
        if self.use_dropout:
        ...
        i = self.num_layers
        dout, grads['W'+str(i)], grads['b'+str(i)] = affine_backward(dScores, caches[i-1])
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            if self.normalization=='batchnorm':
                dout, grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)] = affine_relu_bn_backward(dout, caches[i-1])
            elif self.normalization=='layernorm':
                dout, grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)] = affine_relu_ln_backward(dout, caches[i-1])
            else:
